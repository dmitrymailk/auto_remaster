...
with use_local_python():
    ...
    class StreamDiffusionWrapper:
        ...
        def preprocess_image(self, image: Union[str, Image.Image]) -> torch.Tensor:
            ...
            if isinstance(image, str):
                image = (
                    Image.open(image).convert("RGB").resize((self.width, self.height))
                )
    ...
    prompt: str = "car driving on the road, natural trees, realistic road"
    ...
    class SobelEdgeDetection(nn.Module):
        def __init__(self):
            ...
            # Register the kernels as buffers so they are not considered model parameters
        ...
        def rgb_to_grayscale(self, x):
            # Assuming x is of shape [B, C, H, W] where C=3 for RGB
            # The standard formula for converting RGB to grayscale:
            # grayscale = 0.2989 * R + 0.5870 * G + 0.1140 * B
            r, g, b = x[:, 0, :, :], x[:, 1, :, :], x[:, 2, :, :]
            grayscale = 0.2989 * r + 0.5870 * g + 0.1140 * b
            return grayscale.unsqueeze(
                1
            )  # Add channel dimension back, shape becomes [B, 1, H, W]

        def forward(self, x):
            # If the input is RGB, convert it to grayscale first
            if x.shape[1] == 3:
    ...
    sobel = SobelEdgeDetection().cuda().to(torch.bfloat16)
...
def on_frame_process(
    ...
    frame = torch.from_numpy(frame).cuda().to(torch.bfloat16)
    # print(frame.device)
    # frame =  255 - frame
    # output_image = stream.stream.image_processor.preprocess(
    #     image=frame,
    #     width=width,
    #     height=height,
    # )
    # print(frame.shape)
    # frame = perspective_transformer(frame)
    frame = sobel(frame.permute(2, 0, 1).unsqueeze(0)).squeeze()
    # frame = stream(image=output_image)
    return frame.cpu().float().unsqueeze(2).repeat(1, 1, 3).numpy()
    # return frame
