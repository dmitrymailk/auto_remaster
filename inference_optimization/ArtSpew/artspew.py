...
DEFAULT_GUIDANCE_SD = 8.0
DEFAULT_GUIDANCE_LCM = 0.0
...
def parse_arguments():
    ...
    parser.add_argument("--xl", action=argparse.BooleanOptionalAction, help="Use SDXL")
    parser.add_argument(
        "-m", "--model-id", type=str, default="auto", help="Specify the input file"
    )
    parser.add_argument(
        "-p", "--prompt", type=str, help="Specify the start of the prompt"
    )
    parser.add_argument(
        "-x",
        "--width",
    ...
    parser.add_argument(
        "-b", "--batch-size", type=int, default=DEFAULT_BATCH_SIZE, help="Batch Size"
    )
    parser.add_argument(
        "-s",
        "--steps",
    ...
    parser.add_argument(
        ...
        "--random-tokens",
        type=int,
        default=DEFAULT_N_RANDOM_TOKENS,
        help="Number of random tokens added",
    )
    parser.add_argument("--no-lcm", action="store_true", help="Use LCM")
    parser.add_argument("--no-tiny-vae", action="store_true", help="Use the tiny VAE")
    parser.add_argument(
        "-g",
        "--guidance",
        type=float,
    ...
    parser.add_argument(
        "--seed", type=int, default=DEFAULT_SEED, help="Random seed, -1 for random"
    )
...
def main():
    ...
    files = [
        entry.name
        for entry in os.scandir("spew")
        if entry.name.startswith(artspew.get_filename_prefix())
    ]
    ...
    for image in image_generator:
        ...
        image.save(
            f"spew/{image.filename_prefix}{sequence_number:09d}-{safe_prompt}.jpg"
        )
