...
def main(args):
    ...
    # encoder-depth=8
    model: SiT = SiT_models[args.model](
    ...
    for epoch in range(args.epochs):
        ...
        for raw_image, y in train_dataloader:
            ...
            with accelerator.accumulate(
                [model, vae, vae_loss_fn]
            ), accelerator.autocast():
                # 1). Forward pass: VAE
                # просто переводим их во float32 и делим на 127.5
                # imgs.float() / 127.5 - 1.
                ...
                # vae_align_proj_coeff=1.5 по умолчанию
                vae_loss = (
                    vae_loss
                    + args.vae_align_proj_coeff * vae_align_outputs["proj_loss"].mean()
                )
                ...
                # считаем градиенты только для VAE после того как
                # вычислили какие-то фичи на замороженной диффузионной модели
                ...
                if accelerator.sync_gradients:
                    grad_norm_disc = accelerator.clip_grad_norm_(
                        vae_loss_fn.parameters(), args.max_grad_norm
                    )
            ...
            if accelerator.sync_gradients:
                ...
                logs = {
                    ...
                    "reconstruction_loss": accelerator.gather(
                        vae_loss_dict["reconstruction_loss"].mean()
                    )
                    .mean()
                    .detach()
                    .item(),
                    "perceptual_loss": accelerator.gather(
                        vae_loss_dict["perceptual_loss"].mean()
                    )
                    .mean()
                    .detach()
                    .item(),
                    "kl_loss": accelerator.gather(vae_loss_dict["kl_loss"].mean())
                    .mean()
                    .detach()
                    .item(),
                    "weighted_gan_loss": accelerator.gather(
                        vae_loss_dict["weighted_gan_loss"].mean()
                    )
                    .mean()
                    .detach()
                    .item(),
                    "discriminator_factor": accelerator.gather(
                        vae_loss_dict["discriminator_factor"].mean()
                    )
                    .mean()
                    .detach()
                    .item(),
                    "gan_loss": accelerator.gather(vae_loss_dict["gan_loss"].mean())
                    .mean()
                    .detach()
                    .item(),
                    "d_weight": accelerator.gather(vae_loss_dict["d_weight"].mean())
                    .mean()
                    .detach()
                    .item(),
                    "grad_norm_vae": accelerator.gather(grad_norm_vae)
                    .mean()
                    .detach()
                    .item(),
                    "vae_align_loss": accelerator.gather(
                        vae_align_outputs["proj_loss"].mean()
                    )
                    .mean()
                    .detach()
                    .item(),
                    "d_loss": accelerator.gather(d_loss).mean().detach().item(),
                    "grad_norm_disc": accelerator.gather(grad_norm_disc)
                    .mean()
                    .detach()
                    .item(),
                    "logits_real": accelerator.gather(d_loss_dict["logits_real"].mean())
                    .mean()
                    .detach()
                    .item(),
                    "logits_fake": accelerator.gather(d_loss_dict["logits_fake"].mean())
                    .mean()
                    .detach()
                    .item(),
                    "lecam_loss": accelerator.gather(d_loss_dict["lecam_loss"].mean())
                    .mean()
                    .detach()
                    .item(),
                }
            ...
            if global_step % args.checkpointing_steps == 0 and global_step > 0:
                if accelerator.is_main_process:
                    ...
                    original_vae = (
                        unwrapped_vae._orig_mod if args.compile else unwrapped_vae
                    )
                    original_discriminator = (
                        unwrapped_vae_loss_fn._orig_mod.discriminator
                        if args.compile
                        else unwrapped_vae_loss_fn.discriminator
                    )
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--loss-cfg-path", type=str, default="configs/l1_lpips_kl_gan.yaml"
    )
