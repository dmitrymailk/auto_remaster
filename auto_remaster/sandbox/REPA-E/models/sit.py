...
class SiT(nn.Module):
    ...
    def forward(
        ...
        normalized_x = self.bn(image_latents)
        ...
        N, T, D = image_latents.shape

        # timestep and class embedding
        t_embed = self.t_embedder(time_input.flatten())  # (N, D)
        image_labels = self.y_embedder(image_labels, self.training)  # (N, D)
        c = t_embed + image_labels  # (N, D)

        for i, block in enumerate(self.blocks):
            image_latents = block(image_latents, c)  # (N, T, D)
            # на 8 слое блока запускаем классификаторы на фичи из
            # диффузионной модели
            if (i + 1) == self.encoder_depth:
                dino_feature_projections = [
                ...
                if loss_kwargs["align_only"]:
                    break
        image_latents = self.final_layer(
            image_latents, c
        )  # (N, T, patch_size ** 2 * out_channels)
        image_latents = self.unpatchify(image_latents)  # (N, out_channels, H, W)

        # loss computation
        denoising_loss = (
            None
            if loss_kwargs["align_only"]
            else mean_flat((image_latents - model_target) ** 2)
        )

        proj_loss = torch.tensor(0.0, device=image_latents.device)
        bsz = dino_features[0].shape[0]
        for i, (dino_feature, dino_feature_projection) in enumerate(
            zip(dino_features, dino_feature_projections)
        ):
            for dino_feature_j, dino_feature_projection_j in zip(
                dino_feature, dino_feature_projection
            ):
                dino_feature_projection_j = torch.nn.functional.normalize(
                    dino_feature_projection_j, dim=-1
                )
                dino_feature_j = torch.nn.functional.normalize(dino_feature_j, dim=-1)
                proj_loss += mean_flat(
        ...
        return {
            "zs_tilde": dino_feature_projections,
            "model_output": image_latents,
