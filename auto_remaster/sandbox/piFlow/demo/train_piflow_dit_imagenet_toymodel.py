...
from lakonlab.models.architecture import DiTTransformer2DModelMod, PretrainedVAE
...
def parse_args():
    ...
    parser.add_argument("--name", type=str, default="jay", help="ImageNet class name")
    parser.add_argument(
    ...
    parser.add_argument(
        "--out",
        type=str,
        default="viz/piflow_dit_imagenet_toymodel/output.png",
...
def main():
    ...
    vae = PretrainedVAE(
        model_name_or_path="stabilityai/sd-vae-ft-ema", torch_dtype=dtype
    ).to(device)
    vae_scale_factor = 8
    ...
    teacher = (
        GaussianFlow(
            ...
            timestep_sampler=dict(
                type="ContinuousTimeStepSampler", shift=1.0, logit_normal_enable=False
            ),
        )
        .eval()
        .to(device)
