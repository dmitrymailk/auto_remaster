...
class PiFluxPipeline(FluxPipeline, PiFlowMixin):
    ...
    def __init__(
        ...
        assert (
            policy_type in POLICY_CLASSES
        ), f"Invalid policy: {policy_type}. Supported policies are {list(POLICY_CLASSES.keys())}."
        self.policy_type = policy_type
        self.policy_class = (
            partial(POLICY_CLASSES[policy_type], **policy_kwargs)
            if policy_kwargs
            else POLICY_CLASSES[policy_type]
        )

    def _unpack_gm(
        self, gm, height, width, num_channels_latents, patch_size=2, gm_patch_size=1
    ):
        ...
        gm["means"] = (
            gm["means"]
            .reshape(bs, h, w, k, c // (scale * scale), scale, scale)
            .permute(0, 3, 4, 1, 5, 2, 6)
            .reshape(bs, k, c // (scale * scale), h * scale, w * scale)
        )
        gm["logweights"] = (
            gm["logweights"]
            .reshape(bs, h, w, k, 1, scale, scale)
            .permute(0, 3, 4, 1, 5, 2, 6)
            .reshape(bs, k, 1, h * scale, w * scale)
        )
    ...
    @staticmethod
    def _unpack_latents(
        latents, height, width, vae_scale_factor, patch_size=2, target_patch_size=1
    ):
        ...
        latents = latents.reshape(
            batch_size, channels // (scale * scale), height * scale, width * scale
        )
        ...
        with self.progress_bar(total=self.num_timesteps) as progress_bar:
            for i, (t_src, t_dst) in enumerate(zip(timesteps, timesteps_dst)):
                ...
                if self.policy_type == "GMFlow":
                    ...
                    denoising_output = {
                        k: v.to(torch.float32) for k, v in denoising_output.items()
                    }
                    policy = self.policy_class(denoising_output, latents, sigma_t_src)
                    if i < self.num_timesteps - 1:
                        if temperature == "auto":
                            temperature = min(
                                max(0.1 * (num_inference_steps - 1), 0), 1
                            )
                        else:
                    ...
                    denoising_output = denoising_output.reshape(
                        latents.size(0), -1, *latents.shape[1:]
                    )
                ...
                latents = self.scheduler.step(
                    latents_dst, t_src, latents, return_dict=False
                )[0]
