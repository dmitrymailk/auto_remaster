...
def guidance_jit(pos_mean, neg_mean, guidance_scale: float, orthogonal: bool = False):
    ...
    if orthogonal:
        ...
        bias = (
            bias
            - (bias * pos_mean).mean(dim=dim, keepdim=True)
            / (pos_mean * pos_mean).mean(dim=dim, keepdim=True).clamp(min=1e-6)
            * pos_mean
        )
    return bias
...
class GaussianFlow(nn.Module):
    ...
    def __init__(
        ...
        self.denoising = (
            build_module(denoising) if isinstance(denoising, dict) else denoising
        )
        ...
        self.timestep_sampler = build_module(
            timestep_sampler, default_args=dict(num_timesteps=num_timesteps)
        )
    ...
    def sample_forward_transition(
        self, x_t_src, noise, t_src=None, t_tgt=None, sigma_src=None, sigma_tgt=None
    ):
        trans_g = self.forward_transition(
            x_t_src, t_src=t_src, t_tgt=t_tgt, sigma_src=sigma_src, sigma_tgt=sigma_tgt
        )[0]
        ...
        if self.denoising_mean_mode.upper() == "U":
            if isinstance(denoising_output, dict):
                ...
                raise AttributeError(
                    "Unknown denoising output type " f"[{type(denoising_output)}]."
                )
            loss_kwargs.update(u_t=noise - x_0)
        else:
            raise AttributeError(
                "Unknown denoising mean output type " f"[{self.denoising_mean_mode}]."
            )
        loss_kwargs.update(
            x_0=x_0,
            noise=noise,
        ...
        **kwargs,
        ...
        if "flow_shift" in signatures and "use_flow_sigmas" in signatures:
            ...
            if "flow_shift" not in sampler_kwargs:
                sampler_kwargs["flow_shift"] = cfg.get(
                    "shift", self.timestep_sampler.shift
                )
        ...
        for t in timesteps:
            ...
            if use_guidance and guidance_active:
                ...
                bias = guidance_jit(
                    mean_pos, mean_neg, guidance_scale, orthogonal_guidance
                )
    ...
    def forward_u(
        ...
        if use_guidance:
            ...
            if guidance_interval[0] > 0 or guidance_interval[1] < self.num_timesteps:
                ...
                ).reshape([num_batches] + [1] * (bias.dim() - 1))
