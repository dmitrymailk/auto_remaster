...
class FlowMapSDEScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        sigmas = torch.from_numpy(
            1 - np.linspace(0, 1, num_train_timesteps, dtype=np.float32, endpoint=False)
        )
    ...
    def get_shift(self, seq_len=None):
        if self.config.use_dynamic_shifting and seq_len is not None:
            ...
            logshift = (
                seq_len - self.config.base_seq_len
            ) * m + self.config.base_logshift
    ...
    def set_timesteps(self, num_inference_steps: int, seq_len=None, device=None):
        ...
        raw_timesteps = (
            torch.from_numpy(
                np.linspace(
                    1,
                    (self.config.final_step_size_scale - 1)
                    / (num_inference_steps + self.config.final_step_size_scale - 1),
                    num_inference_steps,
                    dtype=np.float32,
                    endpoint=False,
                )
    ...
    def step(
    ...
    ) -> Union[FlowMapSDESchedulerOutput, Tuple]:
        ...
        prev_sample = (alpha_to + sigma_to * m) * model_output + sigma_to * (
            1 - m.square()
