"""
!accelerate launch train_rectified_instaflow_lora.py \
  ...
  --output_dir="/content/drive/MyDrive/rectified_flow_checkpoints" \
  --resolution=512 \
  --learning_rate=1e-5 \
...
def make_text_dataset(
    ...
    dataset_output_path = os.path.join(
        output_dir, f"{pretrained_model_name}_generated_images_dataset"
    )
...
def log_validation(
    ...
    pipeline.scheduler = ReflowScheduler(
        pipeline.scheduler.config.num_train_timesteps, distill=distill
    )
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if not distill:
                    ...
                    timesteps = (
                        torch.ones((bsz,), device=latents.device)
                        * noise_scheduler.config.num_train_timesteps,
                    )
                ...
                for i in range(bsz):
                    noisy_latents.append(
                        (
                            noise[i] * scaled_timesteps[i]
                            + latents[i] * (1 - scaled_timesteps[i])
                        )[None]
                    )
