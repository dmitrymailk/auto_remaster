    ...
    **kwargs,
    ...
    for idx in selected_ids:
        ...
        with torch.no_grad():
            ...
            # In new flow matching: x_start (t=1) is structured_noise
            # We start sampling from t=1.0
            ...
            for i in range(num_steps):
                ...
                # 4. Добавление стохастичности (Bridge Noise) - DISABLED for Classical Flow Matching
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(*l_acc):
                ...
                with torch.no_grad():
                    ...
                    cutoff_radius = diffusion_args.minimal_noise_r + np.random.exponential(scale=1 / 0.1)
                    structured_noise = generate_structured_noise_batch_vectorized(
                        z_source.float(),
                ...
                # Classical Flow Matching with Structured Noise Source
                # x_t = t * x_start + (1-t) * x_end
                # x_start = structured_noise
                # x_end = z_target
                # t = sigmas (interpolates 1 -> 0)
                noisy_sample = sigmas * structured_noise + (1.0 - sigmas) * z_target

                # Ensure first timestep uses structured_noise explicitly
                # for i, t in enumerate(timesteps):
                #     if t.item() == 1000:  # Assuming 1000 is the start timestep
                #         noisy_sample[i] = structured_noise[i]
                ...
                # Target V = x_start - x_end = structured_noise - z_target
                target = structured_noise - z_target
                ...
                loss = latent_loss * diffusion_args.latent_loss_weight
